...
bool fDocsHTML = true;
bool fDocsIgnoreNodoc = true;
bool fDocsPrintInternal = true;
bool fDocsOutputVerboseTypes = true;
...
ArgumentDescription docs_arg_desc[] = {
 ...
 {"ignore-nodoc", ' ', NULL, "Generate documentation, ignoring @chpldoc.nodoc attributes (off by default)", "N", &fDocsIgnoreNodoc, "CHPLDOC_IGNORE_NODOC", NULL},
 {"internal-fns", ' ', NULL, "Generate documentation, while including 'chpl__' methods (off by default)", "N", &fDocsPrintInternal, "CHPLDOC_INTERNAL_FNS", NULL},

 {"verbose-typed", ' ', NULL, "Generate documentation, while including 'chpl__' methods (off by default)", "N", &fDocsOutputVerboseTypes, "CHPLDOC_VERBOSE_TYPES", NULL},
...
static bool isNoDoc(const Decl* e) {
  ...
  if (symbolNameBeginsWithChpl(e)) {
    ...
    return !(attrs &&
             attrs->hasPragma(PragmaTag::PRAGMA_CHPLDOC_IGNORE_CHPL_PREFIX)) && !fDocsPrintInternal;
  }
  return false;
}
...
static bool isNoWhereDoc(const Function* f) {
  if (auto attrs = f->attributeGroup())
    if (attrs->hasPragma(pragmatags::PRAGMA_NO_WHERE_DOC) && !fDocsIgnoreNodoc)
      return true;
  return false;
}
...
static bool hasSubmodule(const Module* mod) {
  for (const AstNode* child : mod->stmts()) {
    if (auto mod = child->toModule())
      if ((mod->visibility() != chpl::uast::Decl::PRIVATE && !isNoDoc(mod)) || fDocsPrintInternal) {
        return true;
    }
  }
  return false;
...
struct RstSignatureVisitor {
  ...
  */
  ...
  bool enter(const FnCall* call) {
    ...
    if (isCalleReservedWord(callee) && !fDocsPrintInternal) {
      os_ << " ";
  ...
  bool enter(const Formal* f) {
    if (f->intent() != Formal::DEFAULT_INTENT || fDocsOutputVerboseTypes) {
  ...
  bool enter(const Function* f) {
    ...
    if (f->thisFormal() != nullptr
        ...
        && f->thisFormal()->storageKind() != Qualifier::CONST_REF || fDocsOutputVerboseTypes) {
    ...
    if (f->returnIntent() != Function::ReturnIntent::DEFAULT_RETURN_INTENT &&
        f->returnIntent() != Function::ReturnIntent::CONST || fDocsOutputVerboseTypes) {
...
struct Args {
  ...
  bool printSystemCommands = false;
  bool noHTML = false;
  bool ignoreNodoc = false;
  bool printInternal = false;
  bool printVerboseTypes = false;
};
...
static Args parseArgs(int argc, char **argv, void* mainAddr) {
  ...
  ret.ignoreNodoc = fDocsIgnoreNodoc;
  ret.printInternal = fDocsPrintInternal;
  ret.printVerboseTypes = fDocsOutputVerboseTypes;
  // add source files
